...
class EDSR(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv_last2 = nn.Conv2d(num_feat, num_feat, 3, 1, 1)
        self.conv_last1 = nn.Conv2d(num_feat, num_feat, 3, 1, 1)
    ...
    def forward(self, x):
        ...
        x = self.gff(torch.cat(local_features, 1))  # manipulation  不应该残差  残差过后一定要有卷积接上
        #res = self.conv_after_body(raw1)

        x = torch.mul(torch.sigmoid(x),sfe1) + sfe1
        x = self.conv_last2(self.conv_last1(x) + raw1)
        x = self.conv_last(self.upsample(x)) + self.upsampleraw(raw)
