...
class EDSR(nn.Module):
    ...
    def __init__(self,
        ...
        self.recu = RDB(self.G0, self.G, self.C)
        # self.rdbs = nn.ModuleList([recu])
        # for _ in range(self.D - 1):
        #     self.rdbs.append(recu)
    ...
    def forward(self, x):
        ...
        for count in range(16):
            ...
            if(count<7):
                ...
                #result.append(x)
                local_features.append(x)
                #hour_features.append(x)
            #  print('run to here')
            else:
                # index = count-8
                # if index>0:

                #     x = hour_features[-index] + x
                x = self.recu(x)
                local_features.append(x)
        raw1 = torch.mul(local_features[5],raw1)
        raw1 = torch.mul(local_features[11],raw1)
        x = self.gff(torch.cat(local_features, 1)) + sfe1
        res = self.conv_after_body(x)


        x = self.conv_last(self.upsample(res))
        raw1 = self.conv_last2(self.upsample2(raw1))
